...
export function calculateTeamRatings(picks: DraftPick[], chemistry = false): TeamRatings {
  ...
  // Club chemistry lifts the team's overall (a cohesion bonus), not the lines.
  const overall = gk * 0.16 + defence * 0.28 + midfield * 0.29 + attack * 0.27 + chemistryBonus(picks, chemistry);
